...
export default function UserDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "detection" | "details" | "relations" | "explanation" | "community"
  >("detection");
...
>>>>>>> ccb4627633edc1429f39ea109cdf750fe2a30493
      }
      ...
      try {
        const domainInfoResponse = await fetchWithTimeout(
          `${WHOIS_API_URL}/api/domain-info`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (domainInfoResponse.ok) {
          ...
          console.log(
            `DEBUG: Full riskAdjustment object:`,
            JSON.stringify(riskAdjustment, null, 2),
          );
          ...
          if (riskAdjustment) {
            ...
            const contextualReduction =
              riskAdjustment.reduction_percentage || 0;
            ...
            if (criticalIndicators.length > 0) {
              ...
              riskScore =
                riskScore + deterministicIncrease - contextualReduction;
              riskScore = Math.max(0, Math.min(100, riskScore));
              riskScore = Math.round(riskScore);

              // SAFETY CHECK: If WHOIS info is unavailable, enforce minimum risk score
              const hasWhoisWarning =
                indicators &&
                indicators.some(
                  (indicator: string) =>
                    typeof indicator === "string" &&
                    indicator.includes("WHOIS Information Unavailable") &&
                    !indicator.includes("CRITICAL"),
                );
              if (hasWhoisWarning && riskScore < 45) {
                riskScore = 45;
                console.log(
                  `⚠️ WHOIS unavailable - enforcing minimum risk score of 45%`,
                );
              }
            ...
            console.log(
              `  Deterministic Flags:`,
              riskAdjustment.deterministic_flags,
            );
          }
        }
      } catch (domainError) {
        ...
        console.log(
          `🚨 CRITICAL: Domain verification service is slow or unavailable`,
        );
        ...
        console.log(`  Original Score: ${originalRiskScore}%`);
        console.log(
          `  Enforced Score: ${riskScore}% (security verification service unavailable)`,
        );
        console.log(`  Status: ${status}`);
        ...
        whoisInfo = {
          error:
            "Background verification service took too long - unable to retrieve domain registration details",
          is_timeout: true,
        };
        dnsRecords = {
          error:
            "Background verification service took too long - unable to check DNS configuration",
          is_timeout: true,
        };
        sslInfo = {
          error:
            "Background verification service took too long - unable to verify SSL certificate",
          is_timeout: true,
        };
      }
        ...
        try {
          ...
          if (riskScore < 40) {
            ...
            await addToWhitelist(
              domain,
              user.id,
              `Safe site detected with ${riskScore}% risk score`,
            );
            ...
            await addToBlacklist(
              domain,
              user.id,
              `Phishing site detected with ${riskScore}% risk score`,
            );
  ...
  return (
    <div className="relative min-h-screen bg-[#0a0a0f] text-gray-100 overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Aurora
          colorStops={["#545BFF", "#8B5CF6", "#0a0a0f"]}
          amplitude={1.0}
          blend={0.6}
        />
      ...
      <div className="relative z-10">
          ...
          <div className="w-full max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between relative z-10">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                shadow-lg shadow-[#5667FF]/20 hover:shadow-[#5667FF]/40 hover:-translate-y-0.5 active:translate-y-0 flex items-center gap-2`}
              >
                <svg
                  width="16"
                  height="16"
        ...
        >
          <button
            onClick={() => setShowLogoutModal(true)}
            className="px-10 py-3.5 bg-[#5667FF] text-white text-lg font-medium rounded-full shadow-xl shadow-[#5667FF]/30 hover:bg-[#4a51e0] transition-transform active:scale-95 flex items-center gap-2"
          >
            <svg
              width="16"
              height="16"
        ...
        {/* Main Content */}
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16 mt-20">
          {/* Title */}
          <h1 className="text-2xl md:text-4xl lg:text-5xl font-bold text-white text-center mb-8 md:mb-16">
            Is This Website <span className="text-[#7B83FF]">Safe</span>? Find
            Out Instantly
          </h1>
          ...
          <div className="max-w-3xl mx-auto mb-20">
            <div className="flex items-start gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Scan any website link and instantly detect phishing threats
            ...
            <form onSubmit={handleScan} className="relative group z-20 mb-6">
              <div
                className={`absolute -inset-0.5 bg-gradient-to-r from-[#545BFF] to-[#b19eef] rounded-xl opacity-30 blur transition duration-1000 group-hover:opacity-60 pointer-events-none ${scanning ? "animate-pulse" : ""}`}
              ></div>
            ...
            <p className="text-gray-500 text-[15px] text-center mt-4">
              ...
              <Link href="/terms" className="text-[#7B83FF] hover:underline">
                terms of service
              </Link>{" "}
              and{" "}
              <Link href="/privacy" className="text-[#7B83FF] hover:underline">
                privacy policy
              </Link>
              .
            </p>
            ...
            <div className="mt-8 flex justify-center">
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-[#1a1a2e]/60 border border-gray-700/50 rounded-full backdrop-blur-sm">
                ...
                <span
                  className={`text-xs font-medium tracking-wide ${apiStatus === "online" ? "text-green-400" : "text-red-400"}`}
                >
                  SYSTEM STATUS:{" "}
                  {apiStatus === "online" ? "OPERATIONAL" : "OFFLINE"}
                </span>
              </div>
            </div>
                      ...
                      <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl flex items-start gap-4 text-left">
                        ...
                        <div>
                          ...
                          <p className="text-yellow-200/80 text-sm mt-1">
                            Data sent to this website is not encrypted and could
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mt-6">
                      <div className="bg-[#1a1a2e]/50 p-4 rounded-xl border border-gray-800">
                        ...
                        <div className="text-white font-medium truncate">
                          {currentScan.details?.registrar || "Unknown"}
                        </div>
                      </div>
                      <div className="bg-[#1a1a2e]/50 p-4 rounded-xl border border-gray-800">
                        <div className="text-gray-500 text-xs uppercase tracking-wider mb-1">
                        ...
                        <div className="text-white font-medium">
                          {currentScan.details?.creationDate || "Unknown"}
                        </div>
                      </div>
                      <div className="bg-[#1a1a2e]/50 p-4 rounded-xl border border-gray-800 col-span-2 md:col-span-1">
                        ...
                        <div className="text-white font-medium">
                          {currentScan.details?.lastAnalysisDate}
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-6 md:p-8 bg-gradient-to-b from-[#0a0a0f]/50 to-[#0a0a0f]">
                  ...
                  {activeTab === "details" && (
                    <div className="space-y-4 text-xs md:text-sm">
                      <div>
                        <h4 className="text-gray-400 mb-2 text-xs md:text-sm font-medium">
                          WHOIS Information
                        </h4>
                      ...
                      <div>
                        ...
                        <div className="bg-[#1a1a2e] p-3 md:p-4 rounded border border-gray-800 text-gray-300">
                          ...
                          !currentScan.details.sslCertificates.error ? (
                            ...
                            <p className="text-xs text-center py-4">
                              {currentScan.details?.sslCertificates?.error ||
                                "No SSL certificate data available"}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="space-y-8">
                      {loadingXAI ? (
                        <div className="flex flex-col items-center justify-center py-20 px-4">
                          ...
                          <p className="text-gray-400 font-mono text-sm tracking-wide animate-pulse">
                        ...
                        <>
                          ...
                          {/* Data Matrix */}
                          <div
                            className={`grid gap-6 ${xaiExplanation.risk_factors?.length > 0 && xaiExplanation.positive_factors?.length > 0 ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}
                          >
                            {/* Threat Vector Analysis */}
                            {xaiExplanation.risk_factors &&
                                  ...
                                  <div className="grid gap-3">
                                    {xaiExplanation.risk_factors.map(
                                      (factor: any, idx: number) => (
                                        <div
                                          key={idx}
                                          className="bg-[#1a1a2e] border border-red-500/20 rounded-lg p-4 hover:border-red-500/40 transition-colors group"
                                        >
                            ...
                            {xaiExplanation.positive_factors &&
                                  ...
                                  <div className="grid gap-3">
                                    {xaiExplanation.positive_factors.map(
                                      (factor: any, idx: number) => (
                                        <div
                                          key={idx}
                                          className="bg-[#1a1a2e] border border-green-500/20 rounded-lg p-4 hover:border-green-500/40 transition-colors group"
                                        >
                                          <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                  {historicalData.whois_changes.map(
                                    (change: any, idx: number) => (
                                      <div
                                        key={idx}
                                        className="bg-[#1a1a2e]/50 border border-gray-800 rounded-xl p-4 transition-all hover:border-gray-700"
                                      >
                                              ...
                                              >
                                                ...
                                                <div className="flex flex-col gap-1 text-xs">
                                                  <div className="flex items-center gap-2 text-red-400">
                                                    <span className="w-4 inline-block font-mono">
                                                      -
                                                    </span>
                                                    <span className="font-mono bg-red-500/10 px-1 rounded truncate max-w-full">
                                                      {JSON.stringify(
                                                        fieldChange.from,
                                                      ).replace(/^"|"$/g, "")}
                                                    </span>
                                                  ...
                                                  <div className="flex items-center gap-2 text-green-400">
                                                    ...
                                                    <span className="font-mono bg-green-500/10 px-1 rounded truncate max-w-full">
                                                      {JSON.stringify(
                                                        fieldChange.to,
                                                      ).replace(/^"|"$/g, "")}
                                                    </span>
                                        ...
                                        <div className="space-y-2">
                                          {Object.entries(change.changes).map(
                                            ([recordType, recordChange]: [
                                              ...
                                              <div
                                                key={recordType}
                                                className="text-sm border-l-2 border-[#b19eef]/30 pl-3 py-1"
                                              >
                                                <div className="text-[#b19eef] text-xs font-medium uppercase tracking-wider mb-1">
                                                  {recordType} Records
                                                </div>

                                                {recordChange.added &&
                                                  recordChange.added.length >
                                                    0 && (
                                                    <div className="flex flex-col gap-1 mb-1">
                                                      {recordChange.added.map(
                                                        (
                                                          ...
                                                          >
                                                            <span>+</span>
                                                            <span className="bg-green-500/10 px-1 rounded break-all">
                                                              {val}
                                                            </span>
                                                          </div>
                                                        ),
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                      {recordChange.removed.map(
                                                        (
                                                          ...
                                                          >
                                                            <span>-</span>
                                                            <span className="bg-red-500/10 px-1 rounded break-all">
                                                              {val}
                                                            </span>
                                                          </div>
                                                        ),
                          ...
                          {(!historicalData.whois_changes ||
                            ...
                            (!historicalData.ssl_history ||
                              ...
                              <div className="text-center py-12">
                                <p className="text-gray-400">
                                  No historical changes detected yet.
                                </p>
                                <p className="text-gray-500 text-sm mt-2">
                                  Changes will appear as we track this domain
                                  over time.
                                </p>
                              </div>
                            )}
                          ...
                          {communityComments.length === 0 ? (
                            ...
                            <ul className="mb-6 space-y-4">
                              {communityComments.map((cmt, idx) => (
                                <li
                                  key={idx}
                                  className="bg-[#1a1a2e]/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-5 hover:border-[#7B83FF]/30 transition-all"
                                >
                                  ...
                                  <div className="text-sm text-gray-200 leading-relaxed pl-11">
                                    {cmt.description}
                                  </div>
                                </li>
                              ))}
                            ...
                            <div className="flex gap-2 w-full sm:w-auto">
                              {[
                                ...
                                <button
                                  key={option.id}
                                  onClick={() =>
                                    setSelectedFlag(option.id as any)
                                  }
                                  disabled={submittingComment}
                ...
                <table className="w-full min-w-[640px]">
                  <thead>
                    <tr className="border-b border-gray-800/50 bg-[#0f0f1e]/30">
                      <th className="text-left px-6 py-4 text-gray-500 font-medium text-xs uppercase tracking-wider">
                        URL / Domain
                      </th>
                      <th className="text-left px-6 py-4 text-gray-500 font-medium text-xs uppercase tracking-wider whitespace-nowrap">
                        Time
                      </th>
                      <th className="text-right px-6 py-4 text-gray-500 font-medium text-xs uppercase tracking-wider whitespace-nowrap">
                        Risk Analysis
                      </th>
                    </tr>
                  </thead>
            ...
            <div className="bg-[#0f0f1e] border border-gray-800 rounded-xl p-6 md:p-8 max-w-md w-full shadow-2xl">
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm">
                    Are you sure you want to log out of SmartShield? Your scan
